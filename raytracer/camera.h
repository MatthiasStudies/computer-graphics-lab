...
public:
  Camera(const Vector3df &eye,
    ...
    const float fov_y_rad = static_cast<float>(fov_y_deg * PI / 180.0L);
  ...
  Ray3df generate_ray(const float sx, const float sy) const {
    const Vector3df center = eye_ + image_plane_dist_ * forward_;
    const Vector3df offset = (sx - 0.5f) * image_plane_width_ * right_ +
