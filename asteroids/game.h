...
enum class GameEvent : short
{
    small_asteroid_destroyed, medium_asteroid_destroyed, large_asteroid_destroyed,
...
public:
    Torpedo()
        : Torpedo(Vector2df{0.0f, 0.0f}, 0.0f, Vector2df{1.0f, 1.0f})
    {
    }


    Torpedo(Vector2df position, float angle, Vector2df velocity)
        : TypedBody(BodyType::torpedo,
                    Body2df{
                        BoundingVolume2df{position + 14.0f * Vector2df(angle), 1.0},
...
public:
    ...
    Spaceship(Vector2df position)
        : TypedBody(BodyType::spaceship,
                    Body2df{
                        BoundingVolume2df{position, 10.0f},
...
public:
    ...
    SpaceshipDebris(Vector2df position = Vector2df{0.0, 0.0}, float angle = 0.0)
        : TypedBody(BodyType::spaceship_debris,
                    Body2df{
                        BoundingVolume2df{position, 0.0},
...
public:
    Saucer(short size = 1, Vector2df position = Vector2df{0.0, 0.0},
           std::function<void(Body2df *, float)> saucer_fix = displacement_fix)
        : TypedBody(BodyType::saucer,
                    Body2df{
                        BoundingVolume2df{position, static_cast<float>(size == 1 ? 15 : 7)},
...
public:
    ...
    Debris(Vector2df position = Vector2df{0.0, 0.0}, float angle = 0.0f)
        : TypedBody(BodyType::debris,
                    Body2df{
                        BoundingVolume2df{position, 0.0f},
...
class Game
{
    ...
    static constexpr float SAUCER_SPAN_TIME = 12.0f;
    static constexpr float SHIP_SPAN_TIME = 4.0f;
    static constexpr float ASTEROID_SPAN_TIME = 3.0f;
    ...
    Physics2df physics{
        [&](Body2df* b1, Body2df* b2) -> bool { return this->check_collision(b1, b2); },
        [&](Body2df* b1, Body2df* b2) -> void { this->resolve_collision(b1, b2); }
    };
    Spaceship ship{Vector2df{512.0, 368.0}};
    Saucer saucer{1, Vector2df{70.0, 70.0}, [&](Body2df* body, float time)-> void { this->saucer_fix(body, time); }};
    SpaceshipDebris debris;
    std::array<Asteroid, MAXIMUM_ASTEROIDS_SPAWNING * 4> asteroids; // maximal possible asteroids, in original it is 26
    std::array<Debris, 10> debrises;
    ...
    size_t next_asteroid = 0; // next unused asteroid 
    size_t next_debris = 0; // cycles throw the debrises array
    long long score = 0LL;
    Asteroid* get_next_asteroid();
    Debris* get_next_debris();
    ...
    float saucer_timer = SHIP_SPAN_TIME;
