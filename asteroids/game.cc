...
Asteroid::Asteroid(short size)
    : TypedBody(BodyType::asteroid,
                Body2df{
                    BoundingVolume2df{Vector2df{128.0f + 768.0f * dis(gen), 64.0f + 640.0f * dis(gen)}, size * 11.0f},
...
{
    if (in_hyperspace&& hyperspace_delay
    <=
    0.0 && !is_marked_for_deletion()
    )
    {
