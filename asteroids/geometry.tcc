...
template <class FLOAT, size_t N>
bool AxisAlignedBoundingBox<FLOAT, N>::intersects(AxisAlignedBoundingBox<FLOAT, N> aabb,
                                                  Vector<FLOAT, N> direction) const
{
    AxisAlignedBoundingBox<FLOAT, N> center_extended = AxisAlignedBoundingBox(
        center, half_edge_length + aabb.half_edge_length);

    return center_extended.intersects(Ray{aabb.center, direction});
}

template <class FLOAT, size_t N>
Vector<FLOAT, N> AxisAlignedBoundingBox<FLOAT, N>::sweep_intersects(AxisAlignedBoundingBox<FLOAT, N> aabb,
                                                                    Vector<FLOAT, N> direction) const
{
...
bool Sphere<FLOAT, N>::intersects(const Ray<FLOAT, N>& ray, Intersection_Context<FLOAT, N>& context) const
{
    ...
    if (origin_inside)
    {
        context.normal = static_cast<FLOAT>(-1.0) * context.normal;
        // ray starts inside sphere, normal points to the inside;
    }
    return true;
}

template <class FLOAT, size_t N>
Triangle<FLOAT, N>::Triangle(Vector<FLOAT, N> a, Vector<FLOAT, N> b, Vector<FLOAT, N> c, Vector<FLOAT, N> na,
                             Vector<FLOAT, N> nb, Vector<FLOAT, N> nc)
...
template <class FLOAT, size_t N>
bool Triangle<FLOAT, N>::intersects(const Ray<FLOAT, N>& ray, Vector<FLOAT, N>& normal, Vector<FLOAT, N>& p, FLOAT& u,
...
template <class FLOAT, size_t N>
bool refract(FLOAT refraction_index, Vector<FLOAT, N> normal, Vector<FLOAT, N> direction,
             Vector<FLOAT, N>& transmission)
{
