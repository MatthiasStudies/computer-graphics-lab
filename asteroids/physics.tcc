...
template <class FLOAT_TYPE, size_t N>
BoundingVolumeHyperRectangle<FLOAT_TYPE, N>::BoundingVolumeHyperRectangle(
    Vector<FLOAT_TYPE, N> position, Vector<FLOAT_TYPE, N> edge_lengths)
...
void Body<FLOAT_TYPE, N, BV>::accelerate(FLOAT_TYPE acceleration, FLOAT_TYPE seconds)
{
    if (N >= 2)
    {
        Vector<FLOAT_TYPE, N> velocity = this->velocity + seconds * acceleration * Vector<FLOAT_TYPE, N>{
...
template <class FLOAT_TYPE, size_t N, class BV>
Physics<FLOAT_TYPE, N, BV>::Physics (std::function<
    bool(Body<FLOAT_TYPE, N, BV> *, Body<FLOAT_TYPE, N, BV> *)> check_collision
,
...
template <class FLOAT_TYPE, size_t N, class BV>
bool Physics<FLOAT_TYPE, N, BV>::is_area_free_of_bodies(
    BV * area, std::function < bool(Body<FLOAT_TYPE, N, BV> *) > check_body)
{
    for (Body<FLOAT_TYPE, N, BV> * body
    :
    bodies
    )
    {
    ...
    for (auto iterator1 = bodies.begin(); iterator1 != bodies.end(); iterator1++)
    {
        for (auto iterator2 = iterator1 + 1; iterator2 != bodies.end(); iterator2++)
        {
            if ((*iterator1)->bounding.collides((*iterator2)->bounding))
            {
                ...
                {
                    bodies_to_resolve.push_back(
                        std::pair<Body<FLOAT_TYPE, N, BV>*, Body<FLOAT_TYPE, N, BV>*>(*iterator1, *iterator2));
                }
            }
        }
    }
